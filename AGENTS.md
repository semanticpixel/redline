...
Redline is a terminal-native plan annotator for Codex. It intercepts plans via the `PermissionRequest` hook on `ExitPlanMode`, renders them in a fullscreen terminal UI, lets the user select plan content and annotate parsed steps, then sends structured feedback back to Codex.
